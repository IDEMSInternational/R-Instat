...
Public Class RLink
    ...
    ''' <summary> The font for the R script. </summary>
    Public fScript As Font = New Font("Microsoft Sans Serif", 11, FontStyle.Regular)

    ''' <summary> The colour for the R script. </summary>
    Public clrScript As Color = Color.Black

    ''' <summary> The font for output generated by the R script. </summary>
    Public fOutput As Font = New Font(FontFamily.GenericMonospace, 11, FontStyle.Regular)

    ''' <summary> The colour for output generated by the R script. </summary>
    Public clrOutput As Color = Color.Blue

    ''' <summary> The font for the R script comment. </summary>
    Public fComments As Font = New Font("Microsoft Sans Serif", 11, FontStyle.Regular)

    ''' <summary> The colour for the R script comment. </summary>
    ...
    Public Sub OpenDialogFromScript(strNewScript As String)
        ...
        Dim strCommentExtension As String = "# code generated by the dialog,"

        Dim clsRScript As clsRScript = New clsRScript(strNewScript)
        ...
        For iRstatement As Integer = 0 To clsRScript.lstRStatements.Count - 1
            ...
            If iRstatement = 0 Then
                ...
                ElseIf Not IsNothing(clsRScript.lstRStatements(0).clsElement) Then
                    If Not IsNothing(TryCast(clsRScript.lstRStatements(0).clsElement, clsRElementFunction)) Then
                        If Not IsNothing((TryCast(clsRScript.lstRStatements(0).clsElement, clsRElementFunction)).lstObjects) Then
                            If TryCast(clsRScript.lstRStatements(0).clsElement, clsRElementFunction).lstObjects(0).strPrefix <> "" Then
                                strCommentFromDialogue = TryCast(clsRScript.lstRStatements(0).clsElement, clsRElementFunction).lstObjects(0).strPrefix.ToLower
                            End If
                        Else
                            If TryCast(clsRScript.lstRStatements(0).clsElement, clsRElementFunction).strPrefix <> "" Then
                                strCommentFromDialogue = TryCast(clsRScript.lstRStatements(0).clsElement, clsRElementFunction).strPrefix.ToLower
                            End If
                        End If
        ...
        ElseIf strCommentFromDialogue.Contains("count in factor") Then
            lstNewRCodeStructures.RemoveAt(0)
        ...
        ElseIf strCommentFromDialogue.Contains(strCommentExtension & "labels/levels") Then
            dlgLabelsLevels.lstScriptsRCodeStructure = lstNewRCodeStructures
            dlgLabelsLevels.ShowDialog()
        ElseIf strCommentFromDialogue.Contains(strCommentExtension & " view labels/levels") Then
            lstNewRCodeStructures.RemoveAt(0)
    ...
    Private Shared Function ProcessSingleRStatement(clsNewRStatement As RScript.clsRStatement) As RCodeStructure
        Dim clsNewRCodeStructure As RCodeStructure
        clsNewRCodeStructure = GetListRCodeStructures(clsNewRStatement.clsElement)
        ...
        End If
        Return clsNewRCodeStructure
    End Function

    '''--------------------------------------------------------------------------------------------
    ''' <summary>   Returns RCodeStructure from the Current Rstatement </summary>
    '''
    ''' <param name="clsNewRElement">    the RElement to be processed </param>
    '''--------------------------------------------------------------------------------------------
    Public Shared Function GetListRCodeStructures(clsNewRElement As RScript.clsRElement) As RCodeStructure
        ...
        ElseIf TypeOf (clsNewRElement) Is RScript.clsRElementProperty Then
            ...
            If Not IsNothing(clsRElementFunction.lstObjects) Then
                If clsRElementFunction.lstObjects.Count = 1 AndAlso clsRElementFunction.lstObjects(0).strTxt = "data_book" Then
                    clsNewRFunction.strRCommand = "data_book$" & clsRElementFunction.strTxt
                Else
            ...
            Do While intPos < clsRElementFunction.lstParameters.Count
                If TypeOf (clsRElementFunction.lstParameters(intPos)) Is clsRParameter Then
                    clsNewRFunction.clsParameters.Add(GetParameter(clsRElementFunction.lstParameters(intPos), intPos))
                End If
                intPos += 1
            Loop
            Return clsNewRFunction
        End If
        Return clsNewRCodeStructure
    End Function

    '''--------------------------------------------------------------------------------------------
    ''' <summary>   Returns RParameters from the Current Element's RParameter </summary>
    '''
    ''' <param name="clsNewRParameter">    the RElement Parameters to be processed </param>
    '''--------------------------------------------------------------------------------------------
    Public Shared Function GetParameter(clsNewRParameter As clsRParameter, intParamPos As Integer) As RParameter
        Dim clsRParameter As New RParameter

        If TypeOf (clsNewRParameter) Is clsRParameter Then
            Dim clsRParameterFunction As clsRParameter
            clsRParameterFunction = TryCast(clsNewRParameter, clsRParameter)

            If Not TypeOf (clsNewRParameter.clsArgValue) Is clsRElementProperty AndAlso Not TypeOf (clsNewRParameter.clsArgValue) Is clsRElementFunction AndAlso Not TypeOf (clsNewRParameter.clsArgValue) Is clsRElementAssignable Then
                clsRParameter = New RParameter(clsRParameterFunction.strArgName, strParamValue:=clsNewRParameter.clsArgValue.strTxt, iNewPosition:=intParamPos)
            ElseIf TypeOf (clsNewRParameter.clsArgValue) Is clsRElementOperator OrElse TypeOf (clsNewRParameter.clsArgValue) Is clsRElementFunction Then
                clsRParameter = New RParameter(clsRParameterFunction.strArgName, strParamValue:=GetListRCodeStructures(clsNewRParameter.clsArgValue), iNewPosition:=intParamPos)
            ElseIf TypeOf (clsNewRParameter.clsArgValue) Is clsRElementAssignable Then
                Dim clsNewAsignable As clsRElementAssignable
                clsNewAsignable = TryCast(clsNewRParameter.clsArgValue, clsRElementAssignable)

                If Not IsNothing(clsNewAsignable.clsStatement) Then
                    Dim clsNewRCodeStructure As RCodeStructure = ProcessSingleRStatement(clsNewAsignable.clsStatement)
                    clsRParameter = New RParameter(clsRParameterFunction.strArgName, strParamValue:=clsNewRCodeStructure, iNewPosition:=intParamPos)
                    clsRParameter.strArgumentValue = clsNewRCodeStructure.strAssignTo
                End If
            End If
        Else
            If Not TypeOf (clsNewRParameter.clsArgValue) Is clsRElementProperty AndAlso Not TypeOf (clsNewRParameter.clsArgValue) Is clsRElementFunction Then
                clsRParameter = New RParameter("", strParamValue:=clsNewRParameter.clsArgValue.strTxt, iNewPosition:=intParamPos)
            ElseIf TypeOf (clsNewRParameter.clsArgValue) Is clsRElementOperator OrElse TypeOf (clsNewRParameter.clsArgValue) Is clsRElementFunction Then
                clsRParameter = New RParameter("", strParamValue:=GetListRCodeStructures(clsNewRParameter.clsArgValue), iNewPosition:=intParamPos)
            ElseIf TypeOf (clsNewRParameter.clsArgValue) Is clsRElementAssignable Then
                Dim clsNewAsignable As clsRElementAssignable
                clsNewAsignable = TryCast(clsNewRParameter.clsArgValue, clsRElementAssignable)

                If Not IsNothing(clsNewAsignable.clsStatement) Then
                    clsRParameter = New RParameter("", strParamValue:=ProcessSingleRStatement(clsNewAsignable.clsStatement), iNewPosition:=intParamPos)
                End If
            End If
        End If
