...
Public Class dlgOpenSST
    ...
    Public Sub New()
        ...
        'clsRCSVFile.SetRCommand("rio::import")
        'clsRCSVFile.AddParameter("", clsRFunctionParameter:=clsReadCSV)
            ...
            If dlgOpen.ShowDialog() = DialogResult.OK Then
                ...
                If dlgOpen.FileName <> "" Then
                    ...
                    If strFileExt = ".csv" Then
                        ...
                        'ucrBaseOpenSST.clsRsyntax.SetBaseRFunction(clsReadCSV)
                        ...
                        '        ucrBaseOpenSST.clsRsyntax.clsBaseFunction.ClearParameters()
                        '        ucrBaseOpenSST.clsRsyntax.SetBaseRFunction(clsImportRDS)
                        '        ucrBaseOpenSST.clsRsyntax.AddParameter("data_RDS", clsRFunctionParameter:=clsReadRDS)
                        '        strFileType = "RDS"
                        '        ucrInputName.Hide()
                        '        lblName.Hide()
                        '        'ucrInputName.SetName(strFileName, bSilent:=True)
                        ...
                        '        ucrBaseOpenSST.clsRsyntax.SetBaseRFunction(clsReadXL)
                        '        grpCSV.Hide()
                        '        'grpRDS.Hide()
                        '        'grpExcel.Show()
                        '        'txtPreview.Text = ""
                        ...
                        '        ucrBaseOpenSST.clsRsyntax.clsBaseFunction.ClearParameters()
                        '        ucrBaseOpenSST.clsRsyntax.SetFunction("rio::import")
                        '        ucrBaseOpenSST.clsRsyntax.AddParameter("file", Chr(34) & strFilePath & Chr(34))
                        '        grpCSV.Hide()
                        '        'grpExcel.Hide()
                        '        'grpRDS.Hide()
                        '        'grdDataPreview.Show()
