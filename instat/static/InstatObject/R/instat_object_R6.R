...
instat_object$set("public", "replace_instat_object", function(new_instatObj) {
  private$data_objects <- new_instatObj$data_objects 
  ...
  if(!stack_data) {
    if(missing(data_name)) {
      ...
      for ( i in (1:length(private$data_objects)) ) {
        retlist[[names(private$data_objects)[[i]]]] = get_data_objects(i)$get_data_frame(convert_to_character = convert_to_character)
      }
      return(retlist)
    }
    else return(get_data_objects(data_name)$get_data_frame(convert_to_character = convert_to_character))
  }
  else {
    ...
    return(melt(get_data_objects(data_name)$get_data_frame(), ...))
  }
}
)
...
instat_object$set("public", "get_variables_metadata", function(data_name, data_type = "all", convert_to_character = FALSE) { 
  if(missing(data_name)) {
    retlist <- list()
    for ( i in (1:length(private$data_objects)) ) {
      retlist[[names(private$data_objects)[[i]]]] = get_data_objects(i)$get_variables_metadata(data_type = data_type, convert_to_character = convert_to_character)
    }
    return(retlist)
  }
  else return(get_data_objects(data_name)$get_variables_metadata(data_type = data_type, convert_to_character = convert_to_character))
} 
)
...
instat_object$set("public", "get_combined_metadata", function(convert_to_character = FALSE) { 
  retlist <- data.frame()
  for ( i in (1:length(private$data_objects)) ) {
    templist = get_data_objects(i)$get_metadata()
    for ( j in (1:length(templist)) ) {
      retlist[names(private$data_objects)[[i]], names(templist[j])] = templist[[j]]         
    }
  }
...
instat_object$set("public", "get_data_changed", function(data_name) {
  if(missing(data_name)) {
    ...
    return(get_data_objects(data_name)$data_changed)
  }
} 
)
...
instat_object$set("public", "get_variables_metadata_changed", function(data_obj) { 
  if(missing(data_obj)) {
    ...
    return(get_data_objects(data_obj)$variables_metadata_changed)
  }
} 
)
...
instat_object$set("public", "get_metadata_changed", function(data_obj) { 
  if(missing(data_obj)) {
    ...
    return(get_data_objects(data_obj)$metadata_changed)
  }
} 
)
...
instat_object$set("public", "set_data_frames_changed", function(data_name = "", new_val) {
  ...
  else get_data_objects(data_name)$set_data_changed(new_val)
...
instat_object$set("public", "set_variables_metadata_changed", function(data_name = "", new_val) {
  ...
  else get_data_objects(data_name)$set_variables_metadata_changed(new_val)
} 
)
...
instat_object$set("public", "set_metadata_changed", function(data_name = "", new_val) {
  ...
  else get_data_objects(data_name)$set_metadata_changed(new_val)
} 
)
...
instat_object$set("public", "add_columns_to_data", function(data_name, col_name, col_data, use_col_name_as_prefix) {
  ...
  else get_data_objects(data_name)$add_columns_to_data(col_name, col_data, use_col_name_as_prefix = use_col_name_as_prefix)
}
)
...
instat_object$set("public", "get_columns_from_data", function(data_name, col_names, from_stacked_data = FALSE,
  ...
  if(!from_stacked_data) {
    if(!data_name %in% names(private$data_objects)) stop(paste(data_name, "not found"))
    get_data_objects(data_name)$get_columns_from_data(col_names, force_as_data_frame)
  }
  else {
...
instat_object$set("public", "add_model", function(model, model_name = paste("model",length(models)+1)) {
  ...
  private$models[[model_name]] <- model #self$set_models(model_name, model)
}
) 
...
instat_object$set("public", "replace_value_in_data", function(data_name, col_name, index, new_value) {
  ...
  get_data_objects(data_name)$replace_value_in_data(col_name, index, new_value)
} 
)
...
instat_object$set("public", "rename_column_in_data", function(data_name, column_name, new_val) {
  ...
  get_data_objects(data_name)$rename_column_in_data(column_name, new_val)
} 
)
...
instat_object$set("public", "remove_columns_in_data_from_start_position", function(data_name, start_pos, col_numbers) {
  ...
  get_data_objects(data_name)$remove_columns_in_data_from_start_position(start_pos = start_pos, col_numbers = col_numbers)
} 
)
...
instat_object$set("public", "remove_columns_in_data", function(data_name, cols) {
  ...
  get_data_objects(data_name)$remove_columns_in_data(cols = cols)
} 
)
...
instat_object$set("public", "remove_rows_in_data", function(data_name, start_pos, num_rows) {
  ...
  get_data_objects(data_name)$remove_rows_in_data(start_pos  = start_pos, num_rows = num_rows)
} 
)
...
instat_object$set("public", "get_next_default_column_name", function(data_name, prefix) {
  ...
  return(get_data_objects(data_name)$get_next_default_column_name(prefix))
} 
)
...
instat_object$set("public", "get_column_names", function(data_name) {
  ...
  if(!is.character(data_name)) stop("data_name must be of type character")
  if(!data_name %in% names(private$data_objects)) stop(paste("dataframe: ", data_name, " not found"))
  return(names(get_data_objects(data_name)$data))
}
)
...
instat_object$set("public", "rename_dataframe", function(data_name, new_value = "") {
  ...
  for (i in 1:length(names(data_objects))){
    if(names(data_objects)[i] == data_name){
      names(data_objects)[i] <<- new_value # need a set data objects function 
    }
  }
