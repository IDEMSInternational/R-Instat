...
instat_object$set("public", "add_climdex_indices", function(data_name, indices = list(), freq = "annual", return_df = FALSE) {
  #if (!(year_label)%in% names(data_name))stop("yearly label is missing, create it.")
  #if (!(monthly_label)%in% names(data_name))stop("Mothly label is missing, create it.")
  ...
  if((names(indices) %in% pure_monthly) || ((names(indices) %in% mix_monthly_annual) && freq=="monthly")){
    if (!(paste(data_name, "monthly", sep = "_"))%in% self$get_data_names()){
      ...
      #yy1 = setDT(yy, keep.rownames = TRUE)
      names(yy1) = c("Year_month", names(indices))
      data_list = list(yy1)
      names(data_list) = paste(data_name, "monthly", sep = "_")
      self$import_data(data_tables = data_list)
      
      # dlgSplitText code generated by the dialog Split Text Column
      ...
      #Split <- str_split_fixed(string=Year_month, n=2, pattern="-")
      self$add_columns_to_data(col_data=str_split_fixed(string=Year_month, n=2, pattern="-"), use_col_name_as_prefix=FALSE, data_name=paste(data_name, "monthly", sep = "_"), col_name=c("Year","Month"))
