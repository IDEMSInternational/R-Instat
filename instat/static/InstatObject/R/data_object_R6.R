...
data_object$set("public","split_date", function(data_name, col_name = "", week = FALSE, month_val = FALSE, month_abbr = FALSE, month_name = FALSE, weekday_val = FALSE, weekday_abbr = FALSE, weekday_name = FALSE, year = FALSE, day = FALSE, day_in_month = FALSE, day_in_year = FALSE, leap_year = FALSE, day_in_year_366 = FALSE, decade = FALSE) {
  ...
  if(decade) {
    decade_vector <- as.integer(decade(col_data))
    col_name <- next_default_item(prefix = "decade", existing_names = self$get_column_names(), include_index = FALSE)
    self$add_columns_to_data(col_name = col_name, col_data = decade_vector)
  }
	if(leap_year) {
