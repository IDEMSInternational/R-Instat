  ...
  # A merge is now required because the data is at a different "level"
    ...
    if (exists("col_name")){
      ...
      if (any(stringr::str_detect("ordered", col_data_type))){
        ...
        if (any(grepl("summary_count_name|summary_count_miss|summary_count_all|summary_min|summary_max|summary_range|summary_median|summary_quantile|p10|p20|p25|p30|p33|p40|p60|p67|p70|p75|p80|p90", formula_fn_exp))){
          curr_data_list[[c_data_label]] <- curr_data_list[[c_data_label]] %>%
        ...
        if (any(grepl("summary_count_name|summary_count_miss|summary_count_all", formula_fn_exp))){
          curr_data_list[[c_data_label]] <- curr_data_list[[c_data_label]] %>%
            dplyr::summarise(!!calc$result_name := !!rlang::parse_expr(calc$function_exp)) 
        } else {
          curr_data_list[[c_data_label]] <- curr_data_list[[c_data_label]] %>%
