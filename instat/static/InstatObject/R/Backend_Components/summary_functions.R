...
data_obj$methods(merge_data = function(new_data, by = NULL, type = "left", match = "all") {
  ...
  append_to_changes(Merged_data)
}
)

instat_obj$methods(append_summaries_to_data_object = function(out, data_name, columns_to_summarise, summaries, factors = c(), summary_name) {
...
instat_obj$methods(calculate_summary = function(data_name, columns_to_summarise, summaries, factors = c(), store_results = TRUE, drop = FALSE, return_output = TRUE,summary_name,...) {
  if(!is.character(data_name)) stop("data_name must be of type character")
...
data_obj$methods(calculate_summary = function(columns_to_summarise, summaries, factors = c(), store_results = TRUE, drop = FALSE,...) {
